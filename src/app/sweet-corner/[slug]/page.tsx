    ...
    <>
      ...
      <section className="container-site py-14 sm:py-20">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:items-start">
          <div>
            ...
            <h1 className="mt-6 text-4xl font-bold text-brown sm:text-5xl">{sweet.name}</h1>
          ...
          <div className="flex flex-col gap-6">
            <div className="rounded-2xl border border-gold/20 bg-white p-6">
              ...
              <ul className="mt-3 flex flex-wrap gap-2">
                {sweet.packSizes.map((size) => (
                  <li key={size} className="rounded-full bg-cream px-3 py-1.5 text-sm font-medium text-brown">
                    {size}
                  </li>
                ))}
              </ul>
            ...
            <div className="rounded-2xl border border-gold/20 bg-white p-6">
              <h2 className="text-lg font-bold text-brown">Best For</h2>
      ...
      <section className="bg-cream py-14 sm:py-20">
        <div className="container-site">
          <h2 className="text-2xl font-bold text-brown sm:text-3xl">More From Sweet Corner</h2>
          <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-3">
            {related.map((item) => (
              <Link
                key={item.slug}
                href={`/sweet-corner/${item.slug}`}
                className="group flex flex-col rounded-2xl border border-gold/20 bg-white p-6 transition-all hover:-translate-y-1 hover:border-gold hover:shadow-lg hover:shadow-gold/10"
              >
